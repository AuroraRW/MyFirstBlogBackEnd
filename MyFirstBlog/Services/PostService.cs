...
namespace MyFirstBlog.Services
{
    public class PostService
    {
        ...
        {
            using var connection = new NpgsqlConnection(_connectionString);
            var post = connection.QuerySingleOrDefault<Post>("SELECT * FROM Posts WHERE Slug = @Slug", new { Slug = slug });
            if (post == null)
                return null;
        ...
        public PostDto CreatePost(CreatePostDto createPostDto)
        {
            ...
            using var connection = new NpgsqlConnection(_connectionString);
            connection.Execute("INSERT INTO Posts (Id, Title, Slug, Body, CreatedDate) VALUES (@Id, @Title, @Slug, @Body, @CreatedDate)", newPost);
